...
(defrecord Vertex 
  [label neighbors latitude longitude status distance cost-estimation component])
                      ...
                      (when-not (graph-vertex-visited? graph (:label vertex))
                        ...
                        (doseq [neighbor-label
                          ...
                          (let [neighbor (graph-get-vertex graph neighbor-label)
                                ...
                                distance (+ @(:distance vertex) 
                                            weight)
                                ; distance from neighbor to the destination vertex and accumulated weights of edges
                                cost-estimation (+ distance (graph-great-circle-distance graph neighbor-label start))]
                            ; sets the distance if one wasn't 
                            ; set before or updates distance if the 
                            ; distance would improve
                            (when (or (zero? @(:distance neighbor))
                                      (< cost-estimation @(:distance neighbor) @(:cost-estimation neighbor)))
                              (dosync
                                (ref-set (:cost-estimation neighbor) 
                                         cost-estimation)
                                (ref-set (:distance neighbor)
                                         distance)))
                            (if (graph-vertex-in-queue? graph neighbor-label)
                              (dlist-bubble-swap! queue neighbor-label cost-estimation)
                              (dlist-insert-priority! queue neighbor-label cost-estimation))
                            (dosync (ref-set (:status neighbor) vertex-status-in-queue))
                            )))
                      ; sets the graph-bfs! continue? flag to true
                      true)))
...
(defn graph-dijkstra! [graph start finish]
  (println "----------Dijkstra's algorithm without weights!----------")
  (newline)
...
(defn graph-dijkstra-with-weights! [graph start finish]
  (println "----------Dijkstra's algorithm with weights!----------")
  (newline)
  (println "++++++++++"start"to"finish"++++++++++")
  (newline)
  (if (and (graph-has-vertex? graph start)
