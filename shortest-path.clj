    ...
    (if (> (count x) 0)
      (do
        ...
        (if (= (subs (first x) 0 1) "X") 
            (do 
              (ref-set string (str @string "take " edge " on the " (subs f 3 12) " highway crossing. Next, you ")) 
              (ref-set string (str @string "choose " edge " in the "(subs f 3 12) " highway crossing. After passing it, "))) 
        (if (even? (count x))
          (if (even? (rand-int 10))
            (ref-set string (str @string "via " edge " drive to " f ". After getting there, ")) 
            (ref-set string (str @string "through " edge " continue to " f ". From " f ", "))) 
          (if (even? (rand-int 10))
            (ref-set string (str @string "take " edge " and drive to " f ". Then, ")) 
            (ref-set string (str @string "via " edge " continue to " f ". When you get to " (first x) ", "))))))
        (recur (rest x)))
      (ref-set string (str @string "take the road to " finish" and you will arrive at the finish!")))))
    (println @string)))
